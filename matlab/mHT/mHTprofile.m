function [real_part, imag_part] = mHTprofile(nu0, GammaD, Gamma0, Gamma2, Delta0, Delta2, NuOptRe, NuOptIm, nu, varargin)
    % ---------------------------------------- 
    ...
    if abs(c2) > 1.0e-9 % Limit where speed dependence impact is lower than numerical noise level
        ...
        if abs(Y)>abs(X)*1.0e-15 % Numerical zero
            z2 = sqrt(X+Y)+csqY;   
            if abs(X)>abs(Y)*3e-8 %
                z1 = z2-2*csqY;
            else
            ...
            w1 = cpf_accurate(-imag(z1), real(z1));
            w2 = cpf_accurate(-imag(z2), real(z2));
            A  = 1.772453850905516/nuD*(w1-w2);
        else
            if abs(sqrt(X)) < 4000 % Numerical infinity
                rX = sqrt(X);
                wX = cpf_accurate(-imag(rX), real(rX));
