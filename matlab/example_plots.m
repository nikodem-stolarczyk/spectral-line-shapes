...
x  = -5*GammaD + nu0:GammaD/100:5*GammaD + nu0;
yR_Ar = zeros(1,length(x));
yI_Ar = zeros(1,length(x));
yR_He = zeros(1,length(x));
yI_He = zeros(1,length(x));
for i=1:length(x)
    [yR_Ar(i), yI_Ar(i)] = mHTprofile(nu0, GammaD, Gamma0_Ar, Gamma2_Ar, Delta0_Ar, Delta2_Ar, NuOptRe_Ar, NuOptIm_Ar, x(i));
    [yR_He(i), yI_He(i)] = mHTprofile(nu0, GammaD, Gamma0_He, Gamma2_He, Delta0_He, Delta2_He, NuOptRe_He, NuOptIm_He, x(i));
end
