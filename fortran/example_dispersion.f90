...
!----------------------------------------------------------------------!
! This simple program provides the dispersion profile call specific for
...
!----------------------------------------------------------------------!
   real(dp) :: mHT
      ! Spectral line shape function (in cm); dispersion profile is
      ! obtained as the imaginary part of the profile() function:
      ! "aimag(profile(...))"
...
!----------------------------------------------------------------------!
   nu  = nu0 + 1.0_dp
   mHT = aimag(profile(nu0,GamD,Gam0,Gam2,Shift0,Shift2,NuOptRe,NuOptIm,nu))
